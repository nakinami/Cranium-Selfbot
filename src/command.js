...
class Command {
  ...
  process (msg, args) {
    ...
    if (this.perms.length > 0) {
      if (!msg.channel.guild) {
        this.send(msg, 'this is a server only command (dont use in pms)', 5000)
        return
      }
      const authorPerms = msg.channel.permissionsOf(msg.author.id).json
      for (let perm in this.perms) {
        if (!authorPerms[this.perms[perm]]) {
          this.send(msg, `you dont have permission. (${this.perms[perm]})`, 5000)
          return
        }
      }
    }
    ...
    try {
      ...
      this.send(msg, 'bot machine broke', 5000)
      return
    }
  }
  ...
  send (msg, content, deleteDelay = 0) {
    ...
    if (content.length > 20000) {
      this.log.err('message is too big (2000+ chars)')
      return
    }
